...
Auth getAuthInstance(App app) {
  ...
  // Rider Web intentionally uses session persistence. IndexedDB-first
  // initialization can fail to emit the initial auth state in affected
  // browsers, leaving Firebase.initializeApp waiting indefinitely.
...
class Auth extends JsObjectWrapper<auth_interop.AuthJsImpl> {
  ...
  Future<void> onWaitInitState() async {
    await jsObject.authStateReady().toDart;
    _initUser = User.getInstance(jsObject.currentUser);
  }
