...
class _RiderJobOfferScreenState extends State<RiderJobOfferScreen> {
  ...
  void _refreshOffers() {
    _offersFuture = FirebaseFunctions.instanceFor(region: 'us-central1')
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (offers.isEmpty) {
                      return _JobsStateScaffold(
                        ...
                        onAction: () => setState(_refreshOffers),
                      );
                    }
...
class _StateScaffold extends StatelessWidget {
  final String title;
  final String message;
  const _StateScaffold({required this.title, required this.message});
...
class _AcceptedTopPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const priority = ['Health+', 'Gift', 'Vanguard', 'Heavy Duty', 'Scheduled'];
    final source = chips.where((chip) => chip != 'Standard').toSet();
    final labels = [
      ...priority.where(source.contains),
      ...source.where((chip) => !priority.contains(chip)),
    ].take(5).toList();
